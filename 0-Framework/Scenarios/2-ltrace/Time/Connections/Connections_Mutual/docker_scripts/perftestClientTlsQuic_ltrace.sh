    ...
    if [ "$USE_TLS" = "true" ]; then
         ...
         if [ "$MUTUAL" = "true" ]; then
           echo "Execution $i - TLS Mutual"
             ...
             # -e 'SSL_*' \
            ltrace -T -f -tt \
              -e 'SSL_connect*+OQS_KEM_decaps*+OQS_KEM_free*+OQS_KEM_new*+OQS_KEM_keypair*+EVP_PKEY_keygen*+EVP_PKEY_decapsulate*+EVP_PKEY_encapsulate*+EVP_PKEY_derive*+EVP_DigestSign*+EVP_PKEY_Sign*+EVP_DigestVerify*+EVP_PKEY_Verify*+X509_verify_cert*' \
