...
def stats(xs):
    ...
    cv = (statistics.stdev(filtered) / mean) if len(filtered) > 1 else 0.0
...
def main():
    ...
    conns = []
    curr = None
    expecting_server_response = False
    got_useful_packet = False  # Indica si hemos visto tráfico útil desde el último reset

    for i, (msg, t, sz) in enumerate(vpn_pkts):
       
        if msg == 'P_CONTROL_HARD_RESET_CLIENT_V2':
            if curr:
                conns.append(curr)
            curr = {'start': t, 'sizes': [sz], 'end': None, 'messages': [(msg, t, sz)]}
            expecting_server_response = True
            got_useful_packet = False
        elif msg == 'P_CONTROL_HARD_RESET_SERVER_V2' and curr:
            curr['sizes'].append(sz)
            curr['messages'].append((msg, t, sz))
            expecting_server_response = False
            got_useful_packet = False
        elif curr:
            if msg == 'P_DATA_V2' and curr['end'] is None:
                if not got_useful_packet:
                    # Este P_DATA_V2 es antiguo, no se registra ni cierra la conexión
                    print(f"⚠️  Ignored P_DATA_V2 at t={t:.6f} (likely from previous connection)")
                    continue
                # Este sí es el cierre válido
                curr['sizes'].append(sz)
                curr['messages'].append((msg, t, sz))
                curr['end'] = t
                conns.append(curr)
                ...
                # Cualquier otro mensaje sí se añade
                curr['sizes'].append(sz)
                curr['messages'].append((msg, t, sz))

                if msg in ('P_CONTROL_V1', 'P_ACK_V1'):
                    got_useful_packet = True
    ...
    print("🧪 Detected VPN connections with message trace:")
    for i, c in enumerate(conns, 1):
        start = c['start']
        end = c['end']
        duration = f"{(end - start):.3f} s" if end is not None else "N/A"
        print(f"🔸 Conn {i}: start={start:.6f}, end={end}, duration={duration}, packets={len(c['sizes'])}")
        print("   📦 Messages:")
        for m, t, sz in c['messages']:
            print(f"     - {m:35} at t={t:.6f}, size={sz} bytes")
    ...
    for idx, c in enumerate(conns, start=1):
        ...
        if expected_vpn is not None:
            if abs(vpn_count - expected_vpn) > 1:
