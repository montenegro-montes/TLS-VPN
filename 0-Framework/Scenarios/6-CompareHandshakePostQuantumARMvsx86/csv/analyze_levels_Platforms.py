...
def analyze_signature_csvs(csv_arm, csv_x86, base_name):
    ...
    # Debe contener: KEM, Mean_ARM, Mean_x86, SpeedUp, p_value, etc.
    ...
    cols = [
        "KEM",
        "Mean_ARM","CV_ARM","Outliers%_ARM",
        "Mean_x86","CV_x86","Outliers%_x86",
        "SpeedUp"
    ]
    print("\n📊 ARM vs x86 resumen (sin extremos, con IQR-outliers):")
    print(comparisons_df[cols].round(4).to_string(index=False))

    # --- Export a LaTeX v3 (sin extremos, con IQR-outliers) ---------------
    table_df = comparisons_df[cols].round(2).copy()
    # Nombres “seguros” para iterar:
    table_df.columns = [
        'KEM',
        'ARM_Mean', 'ARM_CV', 'ARM_OutPct',
        'x86_Mean', 'x86_CV', 'x86_OutPct',
        'SpeedUp'
    ]

    tex_path = f"./output/handshake_comparison_{base_name}v3.tex"
    with open(tex_path,'w') as f:
        f.write("\\begin{table*}[!ht]\n")
        f.write("\\begin{tabular}{lrrr|rrr|r}\n")
        f.write(" \\toprule\n")
        f.write("& \\multicolumn{3}{c}{\\bfseries ARM} "
                "& \\multicolumn{3}{c}{\\bfseries x86} &\\\\\n")
        f.write("\\cmidrule(lr){2-4} \\cmidrule(lr){5-7}\n")
        f.write(
            "\\textbf{KEM} & \\textbf{Mean} & \\textbf{CV} & "
            "\\textbf{Out.\\%} & \\textbf{Mean} & \\textbf{CV} & "
            "\\textbf{Out.\\%} & \\textbf{SpeedUp}\\\\\n"
        )
        f.write("\\midrule\n")
        for _, r in table_df.iterrows():
            f.write(
                f"{r['KEM']} & "
                f"{r['ARM_Mean']:.2f} & {r['ARM_CV']:.2f} & {r['ARM_OutPct']:.2f} & "
                f"{r['x86_Mean']:.2f} & {r['x86_CV']:.2f} & {r['x86_OutPct']:.2f} & "
                f"{r['SpeedUp']:.2f}× \\\\\n"
            )
        f.write("\\bottomrule\n")
        f.write("\\end{tabular}\n")
    ...
    print(f"✔ LaTeX v3 created {tex_path}")
    return df_all
