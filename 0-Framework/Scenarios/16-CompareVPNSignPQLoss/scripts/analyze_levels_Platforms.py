...
def generate_adaptive_summary(comparisons_df, outliers_df, base_name):
    ...
    summary_path = f"./{dir_name}/summary_adaptive_{base_name}_{mode}.tex"
...
def analyze_signature_csvs(csv_arm, csv_x86, base_name):
    ...
    tex_path = f"./{dir_name}/handshake_comparison_{base_name}_{mode}v4.tex"
...
def generate_violin_plot(df_all, base_name):
    ...
    ax = sns.violinplot(data=df_filtered, x="KEM", y="Time (ms)", hue="Platform",
                   split=True, inner="quartile", linewidth=1.2, palette="Set2", cut=0, scale="width")

    if mode == "vpn":
        plt.title(f"Impact of KEM on VPN Setup Time ({base_name})", fontsize=26)
    else:
        plt.title(f"Impact of KEM on TLS Handshake Time ({base_name})", fontsize=26)
    ...
    if mode == "vpn":
        plt.ylabel("VPN Time (ms)", fontsize=26)
    else:
    ...
    plt.tight_layout()
    plt.savefig(f"./{dir_name}/violin_{base_name}_{mode}.svg")
    plt.savefig(f"./{dir_name}/violin_{base_name}_{mode}.pdf")
    print(f"📁 Plot saved as: violin_{base_name}_{mode}.*")
    plt.close()


if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python compare_levels_complete.py <csv_arm> <csv_x86>")
        sys.exit(1)
    ...
    if "_arm" not in csv_arm.lower() or "_x86" not in csv_x86.lower():
        print("❌ Error: expected filenames like <sig>_tls_arm.csv and <sig>_tls_x86.csv or <sig>_vpn_arm.csv and <sig>_vpn_x86.csv")
        sys.exit(1)

    # Extract and compare base signature name

   # Detect TLS or VPN mode from file names
    if "_vpn_" in csv_arm and "_vpn_" in csv_x86:
        mode = "vpn"
        sig1 = os.path.basename(csv_arm).split("_vpn_")[0]
        sig2 = os.path.basename(csv_x86).split("_vpn_")[0]
    elif "_tls_" in csv_arm and "_tls_" in csv_x86:
        mode = "tls"
        sig1 = os.path.basename(csv_arm).split("_tls_")[0]
        sig2 = os.path.basename(csv_x86).split("_tls_")[0]
    else:
        print("❌ Error: Inconsistent or unknown filename format.")
        sys.exit(1)
    ...
    print(f"🔍 Signature: {sig1}, Mode: {mode}")
